...
def _agent_view_data(agent: AgentConfig, settings) -> dict:
    ...
    from incubator.tools.knowledge_io import load_objects
...
async def wizard_generate(req: GenerateRequest):
    ...
    system = """\
You are an agent architect for an idea incubator system. Given a description of what \
an agent should do, generate a complete agent configuration as JSON.

The incubator has these agent types:
