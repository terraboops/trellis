...
git () {
	command git "$@"
	local git_exit_code=$? 
	if [[ "$1" == "clone" ]] && [[ $git_exit_code -eq 0 ]]
	then
		local repo_dir
		if [[ "${@: -2:1}" == "-d" ]] || [[ "${@: -2:1}" == "--directory" ]]
		then
			repo_dir="${@: -1}" 
		else
		...
		(
			cd "$repo_dir" 2> /dev/null && command git maintenance start 2> /dev/null &
		)
		[[ $? -eq 0 ]] && echo "✓ Git maintenance enabled for $repo_dir"
	fi
...
with-claude-autonav () {
	CLAUDE_CONFIG_DIR=~/.claude-autonav "$@"
}
with-claude-personal () {
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  ...
  if [[ -n $ZSH_VERSION ]]; then
    ARGV0=rg /Users/terra/Developer/incubator/.venv/lib/python3.14/site-packages/claude_agent_sdk/_bundled/claude "$@"
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]] || [[ "$OSTYPE" == "win32" ]]; then
    ARGV0=rg /Users/terra/Developer/incubator/.venv/lib/python3.14/site-packages/claude_agent_sdk/_bundled/claude "$@"
  elif [[ $BASHPID != $$ ]]; then
    exec -a rg /Users/terra/Developer/incubator/.venv/lib/python3.14/site-packages/claude_agent_sdk/_bundled/claude "$@"
  else
    (exec -a rg /Users/terra/Developer/incubator/.venv/lib/python3.14/site-packages/claude_agent_sdk/_bundled/claude "$@")
  fi
}
fi
export PATH=/Users/terra/.local/bin\:/opt/homebrew/opt/jpeg/bin\:/Users/terra/bin\:/Users/terra/.cargo/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/opt/pmk/env/global/bin
